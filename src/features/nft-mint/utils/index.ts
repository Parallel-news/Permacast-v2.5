...
export const compileShowData = async ({
  pid,
  podcasts,
  nftPayload,
}: compiledShowObject) => {
  const podcast = podcasts.find((obj) => obj.pid === pid); //Grab all podcasts
  const mintedEpisodes = podcast.episodes.map((episode) =>
    nftPayload?.records?.find((record) => record?.eid === episode?.eid)
  ); //which ones minted?
  let countMinted = 0;
  const jointEpisodes = podcast.episodes.map((episode, index) => {
    //create new payload showing mint status
...
export const existsClaimableFactories = (nftPayload: NftObject) => {
  ...
  if (!nftPayload) return { claimableFactories: false }
  if (nftPayload?.claimable_factories) {
