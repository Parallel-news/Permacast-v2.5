...
const FeaturedPodcastCarousel = React.lazy(
  () => import("@/component/reusables/FeaturedPodcastCarousel")
);
...
const FeaturedCreators = React.lazy(
  () => import("@/component/creator/featuredCreators")
);
const FeaturedPodcast = React.lazy(
  () => import("@/component/home/featuredPodcast")
);
const FeaturedChannelModal = React.lazy(
  () => import("@/component/home/featuredChannelModal")
);
const Greeting = React.lazy(() =>
  import("@/component/featured").then((module) => ({
        ...
        {isProduction !== true && (
          <div className="select-text">
            <p className="text-yellow-500 font-bold">
              Heads up: isProduction !== "true"
            </p>
            <div className="text-teal-300 flex gap-x-1">
              EXM Main Address:{" "}
              <p className="underline font-medium">{contractAddress}</p>
            </div>
            <div className="text-indigo-300 flex gap-x-1">
              EXM Feature Channel Address:{" "}
              <p className="underline font-medium">{featuredContractAddress}</p>
            </div>
            <div>
              (PRODUCTION) PASoM Contract Address: {PASoMContractAddress}
            </div>
          </div>
        )}
    ...
    <Suspense fallback={<HomeLoader />}>
      <div className="w-full pb-10 mb-10">
        ...
        {isHomepageLoading ? (
          ...
          isSuccess && (
            <FeaturedPodcastCarousel podcasts={homeData.sortedPodcasts} />
          )
        )}
        <div className="my-9 flexCol xl:flex-row md:justify-between space-y-10 xl:space-y-0">
            ...
            {isHomepageLoading ? (
              ...
              <>
                ...
                <div className="grid grid-rows-3 gap-y-4 text-zinc-100">
                  {homeData.sortedEpisodes.map(
                    (episode: FullEpisodeInfo, index: number) => (
                      <Track
                        {...{ episode }}
                        openFullscreen
                        includeDescription
