...
export default function UploadVideo({ podcast }) {
  ...
  const uploadVideoToArweave = async (data, fileType, vidObj, event, serviceFee) => {
        ...
        <div className="flex items-center justify-center flex-col rounded-xl">
          <div className="py-6 px-10 w-full form-control">
            <form className="" onSubmit={handleVideoUpload}>
              <div className="mb-5">
                <input className="input input-secondary w-full py-3 px-5 bg-zinc-800 border-0 rounded-xl outline-none focus:ring-2 focus:ring-inset focus:ring-white" required pattern=".{3,500}" title="Between 3 and 500 characters" type="text" name="videoName" 
                // placeholder={t("uploadepisode.name")}
                placeholder={'Video name'}
                 />
              </div>
              <div className="mb-5">
                <textarea className="input input-secondary resize-none w-full h-28 pb-12 py-3 px-5 bg-zinc-800 border-0 rounded-xl outline-none focus:ring-2 focus:ring-inset focus:ring-white" required pattern=".{1,5000}" title="Between 1 and 5000 characters" type="text" name="videoShowNotes" 
                placeholder={'Video description'}
                // placeholder={t("uploadepisode.description")}
                ></textarea>
              </div>
              <div className="mb-5 bg-zinc-800 rounded-xl cursor-pointer">
                <input className="opacity-0 absolute z-[-1]" id="file" required type="file" onChange={(e) => onFileUpload(e.target.files?.[0])} name="videoMedia" />
              ...
              <div className="bg-zinc-700 rounded-lg p-4">
                ...
                <span className="text-lg font-bold underline">{(videoUploadFee).toFixed(3)} AR</span>
              </div>
              {videoUploadFee ? (
                <div className="w-80">
                  <p className="text-gray py-3">{videoUploadFee} {t("uploadepisode.toupload")}</p>
                  <div className="bg-zinc-800 rounded-lg w-full">
                    {t("uploadepisode.feetext")}
                    <span className="text-lg font-bold underline">
                      {(videoUploadFee / EPISODE_UPLOAD_FEE_PERCENTAGE).toFixed(3)} AR
                    </span>
                  </div>
                </div>
              ) : null}
