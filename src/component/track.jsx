...
export default function Track({episode, episodeNumber=1, includeDescription=false, includePlayButton=true, color=""}) {
  ...
  const { contentTx, episodeName, creatorName, description, podcastId, objectType, creatorAddress } = episode;
  ...
  return (
    <div className="flex items-center justify-between">
      <div className="flex items-center">
        <img className="w-14 h-14 rounded-lg cursor-pointer" src={'https://arweave.net/'+secondaryData_.cover} alt={episode.episodeName} onClick={() => history.push(url)} />
        <div className="ml-4 flex flex-col">
          <div className="cursor-pointer line-clamp-1 pr-2 text-sm" onClick={() => history.push(url)}>{episode.episodeName}</div>
          <div className="flex items-center">
              ...
              <>
                ...
                <div style={getButtonRGBs(c)} className="ml-1.5 p-1 rounded-full cursor-pointer">
                  <div className="flex items-center min-w-max">
                    ...
                    <Cooyub className="rounded-full" svgStyle="h-2 w-2" rectStyle="h-6 w-6" fill={"#00ff00"} />
                    <p className="text-[8px] pr-1 ml-1 " onClick={() => history.push("/creator/" + creatorAddress)}>@{primaryData_.podcasts.filter((obj) => {
