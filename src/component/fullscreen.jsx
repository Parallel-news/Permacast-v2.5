...
export default function Fullscreen({episode, number}) {
  ...
  return (
    <div className="absolute h-full w-full z-[20] bg-black" onClick={console.log/*`url("${episode?.cover}")`}}> */}>
      <img className="absolute h-full w-full blur-lg opacity-30 object-cover" src={'https://arweave.net/'+secondaryData_.cover} alt={episode?.episodeName} />
      <div className="absolute mt-28 w-full text-center select-text">
        <img className="w-[25%] cursor-pointer mx-auto" src={'https://arweave.net/'+secondaryData_.cover} alt={episode?.episodeName} />
        <div className="mt-5 text-3xl font-bold text-white">{t("fullscreen.episode")} {secondaryData_
