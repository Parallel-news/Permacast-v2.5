...
import {
  episodeDescStyling,
  episodeNameStyling,
} from "../uploadEpisode/uploadEpisodeTools";
...
import {
  ...
  PODCAST_AUTHOR_MAX_LEN,
  PODCAST_AUTHOR_MIN_LEN,
  PODCAST_DESC_MAX_LEN,
  PODCAST_DESC_MIN_LEN,
  PODCAST_NAME_MAX_LEN,
  PODCAST_NAME_MIN_LEN,
  SPINNER_COLOR,
  TOAST_DARK,
  USER_SIG_MESSAGES,
...
const CoverContainer = React.lazy(() =>
  import("./reusables").then((module) => ({ default: module.CoverContainer }))
);
const ExplicitInput = React.lazy(() =>
  import("./reusables").then((module) => ({ default: module.ExplicitInput }))
);
const SelectDropdownRow = React.lazy(() =>
  import("./reusables").then((module) => ({
    default: module.SelectDropdownRow,
  }))
);
const ConnectButton = React.lazy(() =>
  import("../uploadEpisode/reusables").then((module) => ({
    default: module.ConnectButton,
  }))
);
const UploadButton = React.lazy(() =>
  import("../uploadEpisode/reusables").then((module) => ({
    default: module.UploadButton,
  }))
);
const ValMsg = React.lazy(() =>
  import("../reusables/formTools").then((module) => ({
    default: module.ValMsg,
  }))
);
const SelectPodcast = React.lazy(() =>
  import("../../component/uploadEpisode/reusables").then((module) => ({
    default: module.SelectPodcast,
  }))
);
...
export const ShowForm = (props: ShowFormInter) => {
  ...
  useEffect(() => {
    getBundleArFee(String(GIGABYTE)).then(setArseedCostPerGig);
  }, []);
  ...
  useEffect(() => {
    ...
    if (podcastDescription_.length > 0 && podcastCover_ !== null) {
      calculateTotal().then(async (total) => {
        ...
        setUploadCost(
          props.edit ? formattedTotal : formattedTotal + MIN_UPLOAD_PAYMENT
        );
      });
    } else {
      setUploadCost(0);
  ...
  const createShowPayload: UploadPodcastProps | EditPodcastProps = {
    function:
      props.edit && props.rssData.length === 0
        ? "editPodcastMetadata"
        : "createPodcast",
  ...
  async function submitShow(payload: any) {
    // Check Connection
    console.log();
    ...
    // Package EXM Call
    const payloadObj = await packageMEMPayload(payload);
    ...
    try {
      const description = await upload2DMedia(podcastDescription_);
      payloadObj["desc"] = description?.order?.itemId;
      toast.dismiss(toastDesc);
    } catch (e) {
      toast.dismiss(toastDesc);
      console.log(e);
      handleErr(t("errors.descUploadError"), setSubmittingShow);
      return;
    }
    ...
    try {
      ...
      payloadObj["cover"] = cover?.order?.itemId;
      const minCover = await minifyPodcastCover(podcastCover_);
      const fileMini = createFileFromBlob(minCover, "miniCov.jpeg");
      const miniCover = await upload3DMedia(fileMini, fileMini.type);
      payloadObj["minifiedCover"] = miniCover?.order?.itemId;
      toast.dismiss(toastCover);
    } catch (e) {
      toast.dismiss(toastCover);
      console.log(e);
      handleErr(t("errors.coverUploadError"), setSubmittingShow);
      return;
    }

    // Fee to Everpay
    if (!props.edit || props.rssData.length > 0) {
      //Upload Mode or RSS Mode
      ...
      try {
        ...
        toast.dismiss(toastFee);
        console.log(e);
        handleErr(t("error.everpayError"), setSubmittingShow);
        return;
      }
    }
  ...
  useEffect(() => {
    if (props.edit && props.rssData.length === 0) {
      const restoreSavedData = async () => {
        const podcast = props.podcasts.filter(
          (podcast) => podcast.pid === props.selectedPid
        );
        const p = podcast[0];
        ...
        setPodcastCategory_(
          categories_en.findIndex((cat) => cat === p.categories[0])
        );
  ...
  const editCover =
    props.edit && !props.rssData.length ? ARSEED_URL + coverUrl : coverUrl;
      ...
      <div className="flex flex-col justify-center items-center lg:items-start lg:flex-row w-full">
        ...
        <div className="flex flex-col w-[95%] md:w-[75%] lg:w-[50%] space-y-3">
          ...
          <div className="flexYCenter justify-between">
            ...
            />
            {props.edit && (
              <VisibleInput setVisible={setIsVisible} visible={isVisible} />
            )}
          </div>
          {/*
                        Upload
          ...
          <div className="w-full flexColFullCenter">
            ...
            {uploadCost === 0 &&
              podcastDescription_.length > 0 &&
              podcastCover_ && (
                <p className="mt-2 text-neutral-400">
                  {t("uploadshow.calculatingFee")}
                </p>
              )}
            {uploadCost !== 0 &&
              podcastDescription_.length > 0 &&
              podcastCover_ && (
                <p className="mt-2 text-neutral-400">
                  {t("uploadshow.uploadCost") +
                    ": " +
                    Number(uploadCost).toFixed(6) +
                    " AR"}
                </p>
              )}
