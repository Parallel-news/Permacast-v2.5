...
import {
  ...
  EXTENDED_TOAST_TIME,
  SPINNER_COLOR,
  MAX_EPISODES_TO_UPLOAD_AT_ONCE,
...
const descContainerStyling =
  "w-[100%] h-32 rounded-xl bg-zinc-800 flex justify-start items-start focus-within:ring-white focus-within:ring-2";
...
function ImportedEpisodes({
  ...
  const [isUploadingEpisodes, setIsUploadingEpisodes] =
  ...
  const MAX_PAGES = Math.floor(
    (rssEpisodes?.length || 0) / MAX_EPISODES_TO_UPLOAD_AT_ONCE
  );
  ...
  const calculateTotalAndSetEpisodes = async (episodes: rssEpisode[]) => {
    const { knownEpisodeSizes, unknownEpisodeSizes } = await fetchEpisodeSizes(
      episodes
    );
    ...
    const total = knownEpisodeSizes.map(
      (rssEpisode: rssEpisode) =>
        calculateSizeCost(GIGABYTE_COST, Number(rssEpisode.length)) +
        EPISODE_SLIPPAGE
    );
  ...
  const asyncs = async () => {
    ...
    const finalEpisodes = isReverseOrder
  ...
  useEffect(() => {
    const asyncFn = async () => {
      ...
      const savedEpisodeLinksValue = RSSFeedManager.getValueFromObject(
        realPid + "/" + RSSLink
      );
      ...
      if (savedEpisodeLinksValue) {
        ...
        setCurrentPage(
          Math.ceil(parsedLinks.length / MAX_EPISODES_TO_UPLOAD_AT_ONCE) + 1
        );
      } else {
        RSSFeedManager.addValueToObject(
          realPid + "/" + RSSLink,
          JSON.stringify([])
        );
  ...
  const uploadEpisode = async (
    link: string,
    number: number
  ): Promise<uploadEpisodeInter> => {
    try {
      const tx = (await axios.post("/api/arseed/upload-url", { url: link }))
        .data.response;
  ...
  const saveEpisodeToEXM = async (
    rssEpisode: rssEpisode,
    uploadPaymentTX: string,
    count: number
  ) => {
    ...
    const finalPayload = await packageMEMPayload(uploadEpisodePayload);
    ...
    try {
      ...
      const prev = JSON.parse(
        RSSFeedManager.getValueFromObject(realPid + "/" + RSSLink)
      );
  ...
  const startEpisodesUpload = async () => {
    ...
    if (!realPid) {
      ...
      if (!attemptPid) {
        toast.error("Podcast not found, wait a little and try again", {
          style: TOAST_DARK,
        });
    ...
    for (let i = 0; i < currentEpisodes.length; i++) {
      const tx = await payStorageFee(
        Number(currentEpisodes[i].length),
        i,
        address
      );
      paymentTXes.push(tx);
    ...
    const contentTXPromises = currentEpisodes.map(
      (episode: rssEpisode, number: number) =>
        uploadEpisode(episode.link, number)
    );

    const contentTXResults = (await Promise.all(contentTXPromises)).sort(
      (a: uploadEpisodeInter, b: uploadEpisodeInter) => a.number - b.number
    );
    console.log("contentTXResults", contentTXResults);
    toast.dismiss(toastSaving);
    const savingBlockchainToast = toast.loading(
    ...
    for (let i = 0; i < contentTXResults.length; i++) {
      ...
      const EXMUpload = await saveEpisodeToEXM(
        currentEpisodes[i],
        episode.tx,
        i
      );
  ...
  const startEstimating = async () => {
    const toastEstimating = toast.loading(
      t("loadingToast.estimating"),
      PERMA_TOAST_SETTINGS(EXTENDED_TOAST_TIME)
    );
    setIsCalculating(true);
    try {
      const result = await estimateUploadCost(retryEpisodes);
      const nonZeroResults = result.filter(
        (ep: RSSEpisodeEstimate) => Number(ep.size) > 0
      );
      if (nonZeroResults.length === 0) {
        toast.dismiss(toastEstimating);
        toast.error(
          t("errors.failedToEstimate"),
          PERMA_TOAST_SETTINGS(ERROR_TOAST_TIME)
        );
        setIsCalculating(false);
        return false;
      }
      const newEpisodes = retryEpisodes.map(
        (episode: rssEpisode, index: number) => {
          const length = nonZeroResults.find(
            (result) => result.link === episode.link
          );
          return {
            ...episode,
    ...
    >
      {isCalculating && (
        <div style={{ color: DEFAULT_THEME_COLOR }}>
          <PermaSpinner spinnerColor={SPINNER_COLOR} size={5} />
        </div>
      )}
  ...
  const ReverseOrderButton = () => {
    ...
    return (
      ...
      >
        <Icon
          className="h-6 w-6"
          icon={isReverseOrder ? "ARROWUP" : "ARROWDOWN"}
        />
      </button>
    );
  };
  ...
  const Progress = () => {
    ...
    return <ProgressBar progress={String(progress)} colorHex="#FFFF00" />;
  };
  ...
  const TotalCost = () => (
    <p className="mt-2 text-zinc-300">
      {t("uploadshow.uploadCost") +
        ": " +
        Number(totalUploadCost).toFixed(6) +
        " AR"}
    </p>
  );
      ...
      <div className="flexColFullCenter lg:items-start lg:flex-row w-full">
        ...
        <div className="flexCol w-[95%] md:w-[75%] lg:w-[50%] space-y-3">
          ...
          <div className="flexCol gap-y-2">
            ...
            <div className="text-center mt-4 mb-2 font-medium">
              {uploadedCount} / {rssEpisodes?.length || 0} {t("rss.downloaded")}
            </div>
            <Pagination
