...
import {
  ...
  AR_DECIMALS,
  CONNECT_WALLET,
  EPISODE_DESC_MAX_LEN,
  EPISODE_DESC_MIN_LEN,
  EPISODE_NAME_MAX_LEN,
  EPISODE_NAME_MIN_LEN,
  EPISODE_UPLOAD_FEE,
  ERROR_TOAST_TIME,
  EVERPAY_EOA,
  EXTENDED_TOAST_TIME,
  GIGABYTE,
  PERMA_TOAST_SETTINGS,
  SPINNER_COLOR,
  TOAST_DARK,
  TOAST_MARGIN,
  USER_SIG_MESSAGES,
...
const UploadButton = React.lazy(() =>
  import("./reusables").then((module) => ({ default: module.UploadButton }))
);
const EpisodeMedia = React.lazy(() =>
  import("./reusables").then((module) => ({ default: module.EpisodeMedia }))
);
const SelectPodcast = React.lazy(() =>
  import("./reusables").then((module) => ({ default: module.SelectPodcast }))
);
const ConnectButton = React.lazy(() =>
  import("./reusables").then((module) => ({ default: module.ConnectButton }))
);
const MarkDownToolTip = React.lazy(() =>
  import("../reusables/tooltip").then((module) => ({
    default: module.MarkDownToolTip,
  }))
);
const CoverContainer = React.lazy(() =>
  import("../uploadShow/reusables").then((module) => ({
    default: module.CoverContainer,
  }))
);
...
export const buttonColStyling =
  "w-full flex justify-center items-center relative";
export const tipModalStyling =
  "absolute inset-0 bottom-0 flex justify-center items-center z-50 h-full";
export const episodeFormStyling =
  "w-[90%] md:w-[75%] lg:w-[50%] flex flex-col justify-center items-center space-y-4";
export const episodeNameStyling =
  "input input-secondary w-full py-3 pl-5 pr-10 bg-zinc-800 border-0 rounded-xl outline-none focus:ring-2 focus:ring-inset focus:ring-white default-animation ";
export const episodeDescStyling =
  "input input-secondary resize-none w-full h-28 pb-12 py-3 px-5 bg-zinc-800 border-0 rounded-xl outline-none default-animation ";
const descContainerStyling =
  "w-[100%] h-32 rounded-xl bg-zinc-800 flex justify-start items-start focus-within:ring-white focus-within:ring-2 default-animation ";
...
export const EpisodeForm = (props: EpisodeFormInter) => {
  ...
  const { address, ANS, getPublicKey, createSignature, arconnectConnect } =
    useArconnect();
  const { packageMEMPayload } = useCrossChainAuth();
  const connect = () =>
    arconnectConnect(PERMISSIONS, { name: APP_NAME, logo: APP_LOGO });
  ...
  useEffect(() => {
    if (props.edit) {
      const restoreSavedData = async () => {
        const podcast = props.shows.filter(
          (podcast) => podcast.pid === props.pid
        );
        console.log("podcast: ", podcast);
        console.log("eid: ", props.eid);
        const episode = podcast[0]?.episodes.filter(
          (episode) => episode.eid === props.eid
        );
        ...
        if (episode && episode.length) {
          ...
          const description = (await axios.get(ARSEED_URL + ep.description))
            .data;
  ...
  const handleValMsg = (input: string, type: string) => {
    switch (type) {
      case "epName":
        if (
          input.length > EPISODE_NAME_MAX_LEN ||
          input.length < EPISODE_NAME_MIN_LEN
        ) {
      ...
      case "epDesc":
        if (
          input.length > EPISODE_DESC_MAX_LEN ||
          input.length < EPISODE_DESC_MIN_LEN
        ) {
  ...
  const submitEpisode = async (payload: any) => {
    // Check Connection
    ...
    // Package EXM Call
    let epPayload = await packageMEMPayload(payload);

    // Description to Arseeding
    const toastDesc = toast.loading(
      t("loadingToast.savingDesc"),
      PERMA_TOAST_SETTINGS(EXTENDED_TOAST_TIME)
    );
    setProgress(props.edit ? 25 : 16);
    try {
      const description = await upload2DMedia(epDesc);
      epPayload["desc"] = description?.order?.itemId;
      toast.dismiss(toastDesc);
    } catch (e) {
      toast.dismiss(toastDesc);
      console.log(e);
      handleErr(t("errors.descUploadError"), setSubmittingEp);
      return;
    }
    ...
    if (epThumbnail) {
      const toastCover = toast.loading(
        t("loadingToast.savingCover"),
        PERMA_TOAST_SETTINGS(EXTENDED_TOAST_TIME)
      );
      setProgress(props.edit ? 50 : 32);
      try {
        const convertedCover = await createFileFromBlobUrl(
          epThumbnail,
          "thumbnail.txt"
        );
        const cover = await upload3DMedia(convertedCover, convertedCover.type);
        epPayload["thumbnail"] = cover?.order?.itemId;
        toast.dismiss(toastCover);
      } catch (e) {
        toast.dismiss(toastCover);
        console.log(e);
        handleErr(t("errors.coverUploadError"), setSubmittingEp);
        return;
      }
    }
    ...
    if (!props.edit) {
      setProgress(48);
      const toastCover = toast.loading(
        t("loadingToast.savingMedia"),
        PERMA_TOAST_SETTINGS(EXTENDED_TOAST_TIME)
      );
      try {
        const media = await upload3DMedia(epMedia, epMedia.type);
        epPayload["content"] = media?.order?.itemId;
        epPayload["mimeType"] = determineMediaType(epMedia.type);
        toast.dismiss(toastCover);
      } catch (e) {
        toast.dismiss(toastCover);
        console.log(e);
        handleErr(t("errors.mediaUploadError"), setSubmittingEp);
        return;
      }
    }
    ...
    // Pay Upload Fee
    if (!props.edit) {
      const toastFee = toast.loading(
        t("loadingToast.payingFee"),
        PERMA_TOAST_SETTINGS(EXTENDED_TOAST_TIME)
      );
      setProgress(66);
      try {
        ...
        toast.dismiss(toastFee);
        console.log(e);
        handleErr(t("error.everpayError"), setSubmittingEp);
        return;
      }
    }
    const toastSaving = toast.loading(
      t("loadingToast.savingChain"),
      PERMA_TOAST_SETTINGS(EXTENDED_TOAST_TIME)
    );
    ...
    setTimeout(async function () {
      ...
      setTimeout(async function () {
        ...
        toast.success(
          t("success.episodeUploaded"),
          PERMA_TOAST_SETTINGS(ERROR_TOAST_TIME)
        );
        setProgress(100);
        queryPodcastData.refetch();
        const identifier = ANS?.currentLabel ? ANS?.currentLabel : address;
        const { locale } = router;
        router.push(`/creator/${identifier}`, `/creator/${identifier}`, {
          locale: locale,
          shallow: true,
        });
      }, 7500);
  ...
  return (
    <div className={episodeFormStyling + " pb-10"}>
      ...
      {/*Select Podcast*/}
      {address && !props.edit && (
        <SelectPodcast pid={pid} setPid={setPid} shows={props.shows} />
      )}

      {/*Episode Name*/}
      <input
        className={episodeNameStyling}
        required
        pattern=".{3,500}"
      ...
      <div className={descContainerStyling}>
        <textarea
          className={"w-[93%] " + episodeDescStyling}
          required
        ...
        />
        <MarkDownToolTip placement="top" size={40} />
      </div>
      {epDescMsg.length > 0 && <ValMsg valMsg={epDescMsg} className="pl-2" />}
      {/*Episode Media*/}
      {!props.edit && <EpisodeMedia media={epMedia} setMedia={setEpMedia} />}
      {/*Upload Button  */}
      <div className={buttonColStyling}>
        ...
        {address && address.length > 0 && submittingEp && (
          <ProgressBar progress={String(progress)} colorHex="#FFFF00" />
        )}
        <ConnectButton className="w-[75%] md:w-[50%]" />
        {/*Is Visible Input*/}
        {props.edit && !submittingEp && (
          <div className="absolute right-0">
            <VisibleInput setVisible={setVisible} visible={visible} />
          </div>
        )}
      </div>
      <div className="flexFullCenter">
        {uploadCost !== 0 && epDesc.length > 0 && epMedia && (
          <p className="mt-2 text-neutral-400">
            {t("uploadepisode.feetext")} {Number(uploadCost).toFixed(6) + " AR"}
          </p>
        )}
      </div>
    </div>
