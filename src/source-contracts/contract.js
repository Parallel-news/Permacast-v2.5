export async function handle(state, action) {
    ...
    const POD_NAME_LIMITS = state.limitations?.["podcast_name_len"];
    ...
    if (input.function === "editPodcastMetadata") {
      ...
      if (isVisible) {
        ...
        podcast["isVisible"] = isVisible === "yes" ? true : "no";
      }
    ...
    if (input.function === "importPodcast") {
      ...
      const req = await EXM.deterministicFetch(
        `https://arweave.net/goYBf0snGS2qZe0d6f7L19pLF_a2RmjYlbuWLvWmJVo`
      );
      const newState = req.asJSON();
      state = newState;
      state.isPorted = true;
      state.ar_mem_molecule_endpoints = `http://ar.molecule.sh/ar-auth`
      return { state };
    }
  
    if (input.function === "test") {
      state.contract_is_working = true;
      return { state }
    }
          ...
          try {
            ...
            const isValid = (
              await EXM.deterministicFetch(
                `${state.ar_mem_molecule_endpoints}/${owner}/${encodedMessage}/${signature}`,
              )
            )?.asJSON()?.result;
    ...
    async function _ownerToAddress(pubkey) {
          ...
          try {
            const req = await EXM.deterministicFetch(
              `${state.ar_molecule_endpoint}/${decryptedPubKey}`,
            );
