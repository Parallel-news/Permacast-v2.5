...
export const estimateUploadCost = async (episodes: rssEpisode[]): Promise<RSSEpisodeEstimate[]> => {
  const allHaveLength = episodes.every((episode) => episode.length);
  if (allHaveLength) {
    const rssEpisodeEstimate = episodes.map((episode) => ({
      link: episode.link, size: episode.length, type: episode.fileType
    }));
    return rssEpisodeEstimate;
  }
